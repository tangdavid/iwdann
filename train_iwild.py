    ...
    for batch_idx in range(num_iter):
        ...
        if 'IW' in method:
            ...
            loss = torch.mean(
                ...
                (output.narrow(0, 0, data_source.size(0)), label_source) * weights) / 3.0
        else:
...
def main():
    ...
    parser.add_argument('--root_folder', type=str, default='data/iwildcam/', help="The folder containing the datasets and the lists")
    ...
    build_dataset = build_iwild


    source_list = os.path.join(args.root_folder, 'source_train.txt')
    source_path = os.path.join(args.root_folder, 'source_train_dataset.pkl')
    target_list = os.path.join(args.root_folder, 'target_train.txt')
    target_path = os.path.join(args.root_folder, 'target_train_dataset.pkl')
    test_list   = os.path.join(args.root_folder, 'target_test.txt')
    test_path   = os.path.join(args.root_folder, 'target_test_dataset.pkl')
